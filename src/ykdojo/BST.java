...
public class BST {
    public static void main(String[] args) {
        ...
        TreeNodeLCA head2 = createTree(mapping2, 3);
        // This tree is:
        //  head2 = 3
        //        /   \
        //       1     4
        ...
        TreeNodeLCA head3 = createTree(mapping3, 3);
        // This tree is:
        //  head3 = 3
        //        /   \
        //       1     5
        ...
        TreeNodeLCA head4 = createTree(mapping4, 3);
        // This tree is:
        //  head4 = 3
        //        /   \
        //       1     5
    ...
    public static boolean isBST(TreeNodeLCA node) {
        return isBSTRecursive(node, null, null);
    }

    public static boolean isBSTRecursive(TreeNodeLCA node, Integer lowerLimit, Integer upperLimit){
        System.out.println("node[" + node.value + "] lowerLimit[" + lowerLimit + "] upperLimit[" + upperLimit + "]");
        if (lowerLimit != null && node.value < lowerLimit){
            System.out.println("node[" + node.value + "] IS LESS THAN lowerLimit[" + lowerLimit + "], returning false!");
            return false;
        }
        if (upperLimit != null && node.value > upperLimit){
            System.out.println("node[" + node.value + "] IS GREATER THAN upperLimit[" + lowerLimit + "], returning false!");
            return false;
        }
        boolean isRightBST = true, isLeftBST = true;
        if (node.right != null){
 ...
 class TreeNode {
    ...
    public TreeNode(int value, TreeNodeLCA left, TreeNodeLCA right){
        this.value = value;
        this.left = left;
        this.right = right;
    }
